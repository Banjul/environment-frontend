        ...
        <div class="query_item">
          ...
          >
            <el-option
              ...
              :key="distribution.stationCode"
              :label="distribution.stationName"
              :value="distribution.stationCode"
            ></el-option>
          </el-select>
        </div>
      </el-col>
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        data: "/environment/report/report6",
        station: "/environment/station/findAllStations"
      },
      columns: [
        {
          id: 1,
  ...
  methods: {
    ...
    getData() {
      ...
      this.$axios
        .post(this.url.data, {
          station_id: this.district_value,
          time: this.datetime
        })
        .then(res => {
          // console.log(res)
          this.loading=false;
          this.siteData = res.data.functionalAreaMonitorData;
        });
    },
    //获取筛选条件
    getAllStations() {
      this.$axios.get(this.url.station).then(res => {
    ...
    export2Excel() {
      ...
      require.ensure([], () => {
        ...
        export_json_to_excel(tHeader, data, "报表6");
      });
    },
